...
class Triangulation:
    ...
    def reload_ibeacons(self):
        ...
        for ibeacon in ibeacons:
            ...
            self.coordination[mac] = (ibeacon.x, ibeacon.y, ibeacon.height, ibeacon.txpower, ibeacon.n)
        print(self.coordination)


    def rssi2distance(self, mac, rssi): # 蓝牙信号强度转化成距离
        _, _, h, txpower, n = self.coordination[mac]
        #n = 1.72
        #h = 1.5
        #txpower = -59
        d = (abs(rssi) - abs(txpower)) / (10 * n)
        d = pow(10, d)
        if d <= h:
           d = 0
        else:
    ...
    def getCoord(self, request, need_meta=False):
        if time.time() - self.updatetime > 10 * 60:  # 每过一小时自动更新
            self.reload_ibeacons()

        points = []

        for mac in request:
            rssi = float(request[mac])
            #print("rssi",rssi)
            #print("mac",mac)
            ...
            if mac in self.coordination:# or r[-2:] in self.coordination:
                print("\t", mac, rssi, self.rssi2distance(mac, rssi))
                point = [self.coordination[mac][0], self.coordination[mac][1], self.rssi2distance(mac, rssi)]
                points.append(point)
            #else:
            #    print("not found")
